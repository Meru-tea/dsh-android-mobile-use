...
echo "==== [1/6] 基础工具 (Termux / 清华源) ===="
# 原脚本的 `pkg update && pkg upgrade` 会升级 termux-tools,
# 而 termux-tools 的 postinst 会把 sources.list 重置回官方源
# (新版官方源 termux.danyael.xyz 在国内实测仅 ~10 KB/s, 会卡死数小时)。
# 因此这里: ① 强制写清华源 ② 只做 apt-get update + install, 完全不做 upgrade。
TUNA='deb https://mirrors.tuna.tsinghua.edu.cn/termux/apt/termux-main stable main'
SRC="$PREFIX/etc/apt/sources.list"
[ -f "$SRC" ] && cp -f "$SRC" "$SRC.before-tuna" 2>/dev/null
printf '%s\n' "$TUNA" > "$SRC"
export DEBIAN_FRONTEND=noninteractive
apt-get update -o Dpkg::Use-Pty=0 || { echo "!! apt-get update 失败"; exit 1; }
...
# termux-tools 若被顺带升级会重置 sources.list, 这里补写一次
printf '%s\n' "$TUNA" > "$SRC"
echo "  基础工具 OK  (sources.list 已固定为清华源)"
